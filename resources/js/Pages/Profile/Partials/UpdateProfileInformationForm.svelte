...
<FormSection on:submit={updateProfileInformation}>
    ...
    <div slot="form">
        {#if $page.props.jetstream.managesProfilePhotos}
            ...
            {#if photoPreview}
                <div class="rounded-full w-20 h-20 bg-cover bg-center"
                    style="background-image: url('{photoPreview}');">
                </div>
            {/if}

            <button type="button" on:click|preventDefault={selectNewPhoto}>
                {$t('Select A New Photo')}
            </button>

            {#if user?.profile_photo_path}
                <button type="button" on:click|preventDefault={deletePhoto}>
                    {$t('Remove Photo')}
                </button>
            {/if}
        {/if}
